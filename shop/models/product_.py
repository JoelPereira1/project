...
from shop.extensions import bcrypt, user_mixin
...
from sqlalchemy import Column, Integer, String, Boolean, ForeignKey, Text, JSON, DECIMAL
...
class Product(Base):
    ...
    rating = Column(DECIMAL(8, 2), default=5.0)
    ...
    def __flush_insert_event__(cls, target):
        ...
        if current_app.config["USE_ES"]:
            from public.search import Item
...
class ProductTypeAttributes(Base):
    __tablename__ = "product_type_attribute"
    id = Column(Integer, primary_key=True)
    product_type_id = Column(Integer(), ForeignKey('product_type.id'))
    product_attribute_id = Column(Integer(), ForeignKey('product_attribute.id'))
...
class ProductType(Base):
    ...
    is_shipping_required = Column(Boolean(), default=False)
...
class ProductVariant(Base):
    ...
    price_override = Column(DECIMAL(10, 2), default=0.00)
...
class AttributeChoiceValue(Base):
